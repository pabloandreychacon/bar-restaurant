  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-stadium-grey border border-white/5 rounded-sm overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-stadium-dark border-b border-white/10 text-xs font-bold uppercase tracking-widest text-gray-400">
                <th className="px-6 py-4">Nombre (Internal)</th>
                <th className="px-6 py-4">Nombre (Display)</th>
            ...
            <tbody className="divide-y divide-white/5">
              {categories.map((cat) => (
                <tr key={cat.Id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4">{cat.DisplayName}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => handleEdit(cat)} className="p-2 hover:text-stadium-orange transition-colors"><Edit2 size={16} /></button>
                      <button onClick={() => cat.Id && handleDelete(cat.Id)} className="p-2 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm overflow-y-auto">
          <div className="bg-stadium-grey w-full max-w-md border border-stadium-orange/30 rounded-sm p-8 my-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-display font-bold uppercase">{editingCategory ? 'Editar' : 'Nueva'} Categoría</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-white"><X /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Nombre Interno</label>
                <input required value={formData.Name} onChange={e => setFormData({ ...formData, Name: e.target.value })} className="w-full bg-stadium-dark border border-white/10 rounded-sm px-4 py-2 focus:border-stadium-orange outline-none" placeholder="ej. burgers" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Nombre a Mostrar</label>
                <input required value={formData.DisplayName} onChange={e => setFormData({ ...formData, DisplayName: e.target.value })} className="w-full bg-stadium-dark border border-white/10 rounded-sm px-4 py-2 focus:border-stadium-orange outline-none" placeholder="ej. Hamburguesas" />
              </div>
